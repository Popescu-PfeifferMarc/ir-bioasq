...
def load_documents_in_batches(folder_path, batch_size):
    ...
    for idx, filename in enumerate(os.listdir(folder_path)):
        try:
            ...
            if (
                content["pmid"] is None
                or content["title"] is None
                or content["body"] is None
            ):
                continue
        ...
        if len(batch) == batch_size:
            ...
            print(
                f"Loaded {idx + 1} / {total_files} documents, Elapsed time: {pretty_print_duration(elapsed_time)}, Time left: {pretty_print_duration(time_left)}"
            )
    if batch:
        print(
            f"Loaded {total_files} / {total_files} documents in {pretty_print_duration(elapsed_time)}"
        )
...
pmid_list, tfidf_matrix, feature_names = process_documents_in_batches(
    dataset_folder, batch_size=100
)

...
output_smartirs_name = (
    "large_ltc" if smartirs == "Ltc" else "small_ltc" if smartirs == "ltc" else smartirs
)  # special case for Ltc and ltc to avoid casing
output_folder = (
...
def preprocess(input: str):
    return [
        lemmatizer.lemmatize(entry)
        for entry in simple_preprocess(input)
        ...
        output.append(
            {
                ...
                # 'query_bow': query_bow,
                "documents": [
                    {"index": idx, "score": float(sim)} for idx, sim in top_sims
                ],
...
for idx, entry in enumerate(output):
    snippets = []
    logger.info(
        f"Querying snippets: {(idx / len(output) * 100):.2f}% ({doc_idx}/{total_entries})"
    )
    for doc in entry["documents"]:
        doc_idx = doc["index"]
        ...
        for sentence in sentences:
            ...
            snippet_score = sum(
                s
                for term_id, s in sentence_tfidf
                if term_id
                in [dictionary.token2id.get(token) for token in sentence_preprocessed]
            )
            snippets.append(
                {"text": sentence, "pmid": pmid, "score": float(snippet_score)}
            )
...
with open(
    output_folder + "results_" + output_smartirs_name + ".json", "w", encoding="utf-8"
) as outfile:
